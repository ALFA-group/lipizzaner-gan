...
class LipizzanerCGANTrainer(EvolutionaryAlgorithmTrainer):
    """
    ...
    def train(self, n_iterations, stop_event=None):
        ...
        for iteration in range(n_iterations):
            ...
            if self.cc.settings['dataloader']['dataset_name'] == 'celeba' \
                ...
                fitness_samples[0] = to_pytorch_variable(fitness_samples[0].view(self.fitness_sample_size, -1))
            ...
            self.evaluate_fitness(all_generators, all_discriminators, fitness_samples[0], fitness_samples[1], self.fitness_mode)
            self.evaluate_fitness(all_discriminators, all_generators, fitness_samples[0], fitness_samples[1], self.fitness_mode)
            ...
            while self.batch_number < len(loaded):
                if self.cc.settings['dataloader']['dataset_name'] == 'network_traffic':
                    ...
                    input_data_tup = next(data_iterator)
                    input_data = input_data_tup[0]
                    labels = input_data_tup[1]
                    batch_size = input_data.size(0)
                ...
                input_data = self.step(local_generators, attackers, defenders, input_data, labels, self.batch_number, loaded,
                                       data_iterator)
                ...
                if self._discriminator_skip_each_nth_step == 0 or self.batch_number % (
                    ...
                    input_data = self.step(local_discriminators, attackers, defenders, input_data, labels, self.batch_number,
            ...
            if self._enable_selection:
                ...
                self.evaluate_fitness(new_populations[TYPE_GENERATOR], all_discriminators, fitness_samples[0], fitness_samples[1],
                                      self.fitness_mode)
                self.evaluate_fitness(new_populations[TYPE_DISCRIMINATOR], all_generators, fitness_samples[0], fitness_samples[1],
                                      self.fitness_mode)
                ...
                self.evaluate_fitness(local_generators, all_discriminators, fitness_samples[0], fitness_samples[1], self.fitness_mode)
                self.evaluate_fitness(local_discriminators, all_generators, fitness_samples[0], fitness_samples[1], self.fitness_mode)
    ...
    def step(self, original, attacker, defender, input_data, labels, i, loaded, data_iterator):
        self.mutate_hyperparams(attacker)
        return self.update_genomes(attacker, defender, input_data, labels, loaded, data_iterator)
    ...
    def update_genomes(self, population_attacker, population_defender, input_var, labels, loaded, data_iterator):
        ...
        for individual_attacker in population_attacker.individuals:
            ...
            loss = attacker.compute_loss_against(defender, input_var, labels)[0]
    ...
    @staticmethod
    def evaluate_fitness(population_attacker, population_defender, input_var, labels, fitness_mode):
        ...
        for individual_attacker in population_attacker.individuals:
            ...
            for individual_defender in population_defender.individuals:
                fitness_attacker = float(individual_attacker.genome.compute_loss_against(
                    individual_defender.genome, input_var, labels)[0])
    ...
    def generate_random_fitness_samples(self, fitness_sample_size):
        ...
        #sampled_data is tuple (images, labels)
        sampled_data, self.fitness_iterator = get_next_batch(self.fitness_iterator, self.fitness_loaded)
        batch_size = sampled_data[0].size(0)

        if fitness_sample_size < batch_size:
            return (sampled_data[0][:fitness_sample_size], sampled_data[1][:fitness_sample_size])
        else:
            ...
            while fitness_sample_size >= batch_size:
                ...
                sampled_data[0] = torch.cat((sampled_data[0], curr_data[0]), 0)
                sampled_data[1] = torch.cat((sampled_data[1], curr_data[1]), 0)
                fitness_sample_size -= batch_size

            if fitness_sample_size > 0:
                # Concatenate partial batch of data
                curr_data, self.fitness_iterator = get_next_batch(self.fitness_iterator, self.fitness_loaded)
                sampled_data[0] = torch.cat((sampled_data[0], curr_data[0][:fitness_sample_size]), 0)
                sampled_data[1] = torch.cat((sampled_data[1], curr_data[1][:fitness_sample_size]), 0)
