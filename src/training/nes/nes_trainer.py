...
class NaturalEvolutionStrategyTrainer(NeuralNetworkTrainer, ABC):
    ...
    def __init__(
        self, dataloader, network_factory, sigma=0.1, alpha=0.001, population_size=50,
    ):
        """
    ...
    def initialize_populations(self):
        ...
        population_gen = Population(
            individuals=[Individual(genome=gen, fitness=gen.default_fitness)], default_fitness=gen.default_fitness,
        )

        population_dis = Population(
            individuals=[Individual(genome=dis, fitness=dis.default_fitness)], default_fitness=dis.default_fitness,
        )
