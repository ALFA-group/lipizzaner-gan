...
class DbLogger:
    ...
    def create_experiment(self, settings):
        ...
        experiment = {
            "name": name,
            "master": master,
            "topology": {"type": "grid", "width": dim, "height": dim,},
    ...
    def finish_experiment(self, experiment_id):
        ...
        collection.update_one(
            {"_id": ObjectId(experiment_id)}, {"$set": {"end_time": datetime.now(timezone.utc)}}, upsert=False,
        )
    ...
    def add_experiment_results(self, experiment_id, node_name, image_paths, inception_score):
        ...
        collection.update_one(
            ...
            {
                "$push": {
                    "results": {"images": images, "inception_score": inception_score, "mixture_center": node_name,}
                }
            },
            upsert=False,
        )
    ...
    def log_results(
        ...
        log_entry = {
            ...
            "iteration": iteration,
            "grid_position": {"x": neighbourhood.grid_position[0], "y": neighbourhood.grid_position[1],},
        ...
        try:
            ...
            self._logger.error(
                "Could not write log entry to database server, as it was not reachable in {}ms.".format(
                    CONNECTION_TIMEOUT
                )
            )
