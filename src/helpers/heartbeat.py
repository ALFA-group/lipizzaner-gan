...
class Heartbeat(Thread):
    ...
    def run(self):
        ...
        while not self.stopped.wait(HEARTBEAT_FREQUENCY_SEC):
            ...
            if count % 5 == 0:
                neighbours_info = self.node_client.get_client_neighbours()
                self.master.update_neighbours(neighbours_info)
                count = 1
            count += 1
            ...
            if dead_clients:
                ...
                if self.kill_clients_on_disconnect:
                    ...
                    _logger.info("Heartbeat: Dead clients {} but will attempt to reconnect to them".format(dead_clients)) 
                    ...
                    if still_dead_clients != []:
                        ...
                        for client in still_dead_clients:
                            self.master.restart_client(client['port'])
                            time.sleep(25) # give a few seconds in between replacing ports 
