...
def main():
    ...
    cc.settings = {
        "network": {"name": "ssgan_conv_mnist_28x28", "loss": "celoss"},
        "trainer": {"params": {"score": {"cuda": cuda}}},
        "dataloader": {"label_rate": 0.01},
    }
    ...
    output_neurons = 784
    train = False
    dataset = datasets.MNIST(root=os.path.join("./output", "data"), train=train, transform=transform, download=True,)

    balanced_batch_sampler = BalancedLabelsBatchSampler(dataset, num_classes, batch_size, 0.99)
    test_loader = torch.utils.data.DataLoader(dataset=dataset, num_workers=0, batch_sampler=balanced_batch_sampler)

    network_factory = cc.create_instance(cc.settings["network"]["name"], output_neurons, num_classes=num_classes)

    disc = network_factory.create_discriminator()
    checkpoint = torch.load("discriminator.pkl", map_location=device)
    checkpoint_final_layer = torch.load("discriminator_classification_layer.pkl", map_location=device)
