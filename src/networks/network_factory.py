    ...
    def create_discriminator(self, parameters=None, encoded_parameters=None):
        net = SSDiscriminatorNet(
            ...
            Sequential(
                ...
                # nn.BatchNorm2d(self.complexity * 8),
                # nn.LeakyReLU(0.2, inplace=True)
                Weight_norm(nn.Conv2d(3, 96, 3, 1, 1), name='weight'),
                # nn.Dropout2d(0.1),
                nn.LeakyReLU(0.2, inplace=True),
                Weight_norm(nn.Conv2d(96, 96, 3, 2, 1), name='weight'),
                # nn.BatchNorm2d(self.complexity * 2),
                nn.Dropout2d(0.3),
                nn.LeakyReLU(0.2, inplace=True),
                Weight_norm(nn.Conv2d(96, 96, 3, 1, 1), name='weight'),
                # nn.BatchNorm2d(self.complexity * 2),
                # nn.Dropout2d(0.1),
                nn.LeakyReLU(0.2, inplace=True),
                Weight_norm(nn.Conv2d(96, 192, 3, 2, 1), name='weight'),
                # nn.BatchNorm2d(self.complexity * 4),
                nn.Dropout2d(0.3),
                nn.LeakyReLU(0.2, inplace=True),
            ...
            Sequential(Weight_norm(nn.Conv2d(192, self.num_classes + 1, 4, 1, 0), name='weight')),
            self.gen_input_size,
