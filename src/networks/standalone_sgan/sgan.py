"""
Authors: Subhash Nalluru and Jamal Toutouh (toutouh@mit.edu) - www.jamal.es

MLP-based SGAN to train the generator and the discriminator to generate and classify MNIST dataset samples.
"""
import argparse
import os
import numpy as np
...
sys.path.insert(0, str(Path(__file__).absolute().parents[2]))  # To change the folder
...
def load(opt):
    ...
    if opt.dataset_name == "mnist":
        ...
        dataset = datasets.CIFAR10(root=os.path.join("./images"), train=True, transform=transform(), download=True,)
...
def main():
    if str(Path(__file__).absolute()).split("/")[-2] == "standalone_sgan":
        os.chdir(str(Path(__file__).absolute().parents[2]))

    Path("networks/standalone_sgan/logs/").mkdir(parents=True, exist_ok=True)
    Path("networks/standalone_sgan/images").mkdir(parents=True, exist_ok=True)
    directory_or_file_name = int(time.time() * 1000)
    Path(f"networks/standalone_sgan/images/output/{directory_or_file_name}").mkdir(parents=True, exist_ok=True)
    ...
    parser.add_argument(
        "--dataset_name", type=str, default="mnist", help="dataset to be used for our experiments: mnist"
    )
    ...
    os.makedirs("../../data/{}".format(opt.dataset_name), exist_ok=True)
    ...
    for epoch in range(opt.n_epochs):
        for i, (imgs, labels) in enumerate(dataloader):
            ...
            imgs = to_pytorch_variable(transpose_data(imgs, opt))
            labels = to_pytorch_variable(transpose_data(labels, opt))
        ...
        if opt.dataset_name == "mnist":
            shape = (1, 28, 28)
        save_images(
            Variable(gen_imgs), shape, f"networks/standalone_sgan/images/output/{directory_or_file_name}/{epoch}.png",
        )
        logging.info(
        ...
        score = float("-inf")
        calc = ScoreCalculatorFactory().create(opt.dataset_name)
