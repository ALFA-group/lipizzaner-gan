...
class TVDBasedSizeFreeGA:
    ...
    def create_individual(self, individual_class, min_ensmbe_size=3, max_ensmbe_size=8):
        max_ensmbe_size = self.generators_in_path  if max_ensmbe_size > self.generators_in_path else max_ensmbe_size
        ensemble_size = [random.randint(min_ensmbe_size, max_ensmbe_size)]
        ...
        return individual_class(ensemble_size+generators_index+weights)
    ...
    def normalize_weights(self, ind):
        ...
        print(total_sum)
        print(ind[self.generators_in_path+1: self.generators_in_path+1 + ind[0]])
...
# For Crossover
    ...
    def cross_generators_genomes(self,ind1, ind2, cxpoint1, cxpoint2, offset):
        ...
        for gen in range(cxpoint1+offset, cxpoint2+offset):
            ...
            ind1 = self.swap_genes(ind1, gen, idx1)
            ind2 = self.swap_genes(ind2, gen, idx2)
...
def main():
    ...
    toolbox.register('individual', ga.create_individual, creator.Individual,
                     min_ensmbe_size=min_ensmbe_size, max_ensmbe_size=max_ensmbe_size)
    ...
    for ind in pop:
        print(ind)
        print(ga.get_individual_generators(ind))
        print(ga.get_individual_weigths(ind))
