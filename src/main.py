...
def ensemble_optimization(args, cc):
    ...
    if algorithm == "ga":
        if mode == "nreo-gen":
            ...
            print(
                "Error: Evolutionary algorithm generators ensemble creator requires selecting a mode: reo-gen or "
                "nreo-gen "
            )
            sys.exit(-1)
        ...
        if mode in ["random", "iterative"]:
            ...
            max_time_without_improvements = cc.settings["ensemble_optimization"]["params"][
                "max_iterations_without_improvemets"
            ]
            precision = 10
