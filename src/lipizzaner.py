...
class Lipizzaner:
    ...
    def __init__(self, trainer=None):
        ...
        final_seed = get_heuristic_seed(
            self.cc.settings["general"]["seed"], local_private_ip(), ClientEnvironment.port,
        )
    ...
    def run(self, n_iterations, stop_event=None):
        ...
        torch.save(
            generator.net.state_dict(), os.path.join(self.cc.output_dir, "generator.pkl"),
        )
        torch.save(
            discriminator.net.state_dict(), os.path.join(self.cc.output_dir, "discriminator.pkl"),
        )

        if "ssgan" in self.cc.settings["network"]["name"]:
            torch.save(
                discriminator.classification_layer.state_dict(),
                os.path.join(self.cc.output_dir, "discriminator_classification_layer.pkl",),
            )
