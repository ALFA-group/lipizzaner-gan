...
def line_plot(title, filename, target_file=None):
    ...
    for i, grp in enumerate(values_to_group):
        plt.plot(
            x, [float(row[grp["name"]]) for row in results], label=grp["label"], ls=grp["ls"], c=grp["c"],
        )
...
def mode_collapse_plot(filename, target_file):
    ...
    def close_enough(row):
        return (
            tvd(
                np.asarray([float(row["m1"]), float(row["m2"])]),
                np.asarray([float(row["m1_opt"]), float(row["m2_opt"])]),
            )
            < 0.5
        )
        ...
        for m1 in np.linspace(1.0, -1.0, num):
            for m2 in np.linspace(-1.0, 1.0 - k, num - int(round(k * 10))):
                ...
                result[map_to_index(m1)][map_to_index(m2)] = sum(
                    1 if close_enough_to_opt(row) else 0 for row in curr_rows
                ) / len(curr_rows)
...
def discr_collapse_fitness_plots(title, generation, filename, target_file, quadrant):
    ...
    l_1, r_1, l_2, r_2 = (
        float(row["gen_adversarial_l1"]),
        float(row["gen_adversarial_r1"]),
        float(row["gen_adversarial_l2"]),
        float(row["gen_adversarial_r2"]),
    )
...
if __name__ == "__main__":
    ...
    discr_collapse_fitness_plots(
        title="Discriminator collapse - Generation 1",
        generation="0",
    ...
    discr_collapse_fitness_plots(
        title="Discriminator collapse - Generation 100",
        generation="99",
        quadrant=[NEGATIVE, NEGATIVE],
        filename=os.path.join(result_dir, "discriminator_collapse.csv"),
